...
- **Popup says "Authorization finished. Return to the admin tab" but admin still shows Login:** GitHub auth succeeded but the popup lost its link to the admin tab (common after cross-origin redirects). Close the popup and wait a moment — the admin tab picks up the token automatically. If it does not, refresh `/admin/` and try again in an incognito window (browser extensions can interfere).
